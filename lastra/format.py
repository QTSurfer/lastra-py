...
def parse_header(buf: bytes) -> Header:
    ...
    series_row_count = int.from_bytes(buf[6:10], "little", signed=False)
    series_col_count = int.from_bytes(buf[10:14], "little", signed=False)
    events_row_count = int.from_bytes(buf[14:18], "little", signed=False)
    events_col_count = int.from_bytes(buf[18:22], "little", signed=False)
...
def write_header(header: Header) -> bytes:
    ...
    buf[5] = header.flags & 0xFF
    buf[6:10] = (header.series_row_count & 0xFFFFFFFF).to_bytes(4, "little", signed=False)
    buf[10:14] = (header.series_col_count & 0xFFFFFFFF).to_bytes(4, "little", signed=False)
    buf[14:18] = (header.events_row_count & 0xFFFFFFFF).to_bytes(4, "little", signed=False)
    buf[18:22] = (header.events_col_count & 0xFFFFFFFF).to_bytes(4, "little", signed=False)
    return bytes(buf)
